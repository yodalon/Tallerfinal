...
class Conversion3D:
    ...
    def coordenadas_rectangulares(self):
        if self.sistema_original == "Rectangulares":
            return self.x, self.y, self.z
        elif self.sistema_original == "Cilíndricas":
            r_cilindricas = math.sqrt(self.x**2 + self.y**2)
            phi_cilindricas = math.atan2(self.y, self.x)
            return r_cilindricas, self.radianes_a_grados(phi_cilindricas), self.z
    ...
    def coordenadas_cilindricas(self):
        if self.sistema_original == "Rectangulares":
            ...
            r_cilindricas = self.x * math.sin(self.grados_a_radianes(self.z))
            phi_cilindricas = math.atan2(self.y, self.x)
            return r_cilindricas, self.radianes_a_grados(phi_cilindricas), self.z
        else:
            raise ValueError("Sistema de coordenadas original no válido")
